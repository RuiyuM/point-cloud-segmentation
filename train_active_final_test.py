...
from annotator.active.sampler import RandomSelect, EntropySelect, MarginSelect, VCDSelect, Entropy_Greedy_Select, VCDSelect_statistic, Logit_Select_Reverse, Features_Entropy_Greedy_Select_Logit_Mean, No_stage_3_Features_Entropy_Greedy_Select_Logit_Mean, No_stage_1_Features_Entropy_Greedy_Select_Logit_Mean
import setproctitle
import wandb
import pickle
...
def loss_fn(inputs, targets):
    ...
    lov_loss = lovasz_softmax
    loss_dict = {
        'CELoss': ce_loss(inputs, targets),
        'LovLoss': lov_loss(inputs.softmax(dim=1), targets,
                                      ignore=0)
    }

    # Return the sum of all weighted losses
...
def active_learning_inference(model, multi_scale_features, out, num_passes=5, dropout_rate=0.3):
    dropout = nn.Dropout(p=dropout_rate)
    out_list = []
    out_list.append(out)
    # Perform multiple passes with dropout
    for _ in range(num_passes - 1):
        dropped_features = dropout(multi_scale_features)
        out = model.classifier(dropped_features)  # Pass through classifier only
        out_list.append(out)

    # Stack and average logits
    stacked_out = torch.stack(out_list, dim=0)
    averaged_logits = torch.mean(stacked_out, dim=0)

    return {'stacked_predict_logits': averaged_logits, 'stacked_logits': stacked_out}
...
class Trainer:
    ...
    def train_one_epoch(self, tbar, data_cfg):
        ...
        ### later added active
        All_true_selected_label_count = None
        ...
        for cur_it in range(total_it_each_epoch):
            ...
            with amp.autocast(enabled=self.if_amp):
                ...
                if cfgs.MODEL.NAME in ['MinkNet', 'SPVCNN']:
                    new_stats = active_learning_inference(self.model, ret_dict['features'], ret_dict['voxel_predict_logits'], 5, 0.3)
                    voxel_predict_logits_active = new_stats['stacked_predict_logits']
                    stacked_out = new_stats['stacked_logits']
                    stacked_out = stacked_out.permute(1, 0, 2)
                    ...
                    features_before_c = ret_dict['features']
                    ...
                    for i in range(len(batch['name'])):
                        ...
                        if if_active:
                            with torch.no_grad():
                                ...
                                voxel_preds_i = stacked_out[batch['targets'].C[:, 3] == i]
                                ...
                                features_before_c_current = features_before_c[batch['targets'].C[:, 3] == i]
                                features_before_c_1 = features_before_c_current[batch['inverse_map'][i]]
                                ...
                                if self.active_times == 5:
                                    ...
                                    if self.active_method == 'EGS':
                                        ...
                                        self.active_sampler = Features_Entropy_Greedy_Select_Logit_Mean(select_num=self.select_num,
                                                                                   num_classes=self.num_classes,
                                                                                   select_method=self.select_method,
                                                                                   voxel_size=self.active_voxel_size,
                                                                                   max_points_per_voxel=self.max_points_per_voxel,
                                                                                   voxel_select_method=cfgs.ACTIVE.VOXEL_SELECT_METHOD)
                                        self.masks[name], true_stat, pre_stat, true_selected_label_count = self.active_sampler.select(
                                            self.masks[name], batch['raw_coord'][i],
                                            preds_i, true_i, self.selected_class_count, features_before_c_1)
                                        self.point_cloud_stat_true.append(true_stat[0])
                                        self.point_cloud_stat_pre.append(pre_stat[0])
                                        if point_count is None:
                                            point_count = true_stat[0][0]
                                        else:
                                            point_count += true_stat[0][0]
                                        current_count = point_count.sum()
                                        if current_count >= 6000:
                                            if_active = False
                                if self.active_method == 'VCD_ST':
...
def main():
    ...
    # args.extra_tag = args.set_cfgs[-1]
